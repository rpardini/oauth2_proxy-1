...
func (p *MattermostProvider) GetEmailAddress(s *sessions.SessionState) (string, error) {
	if s.AccessToken == "" {
...
func (p *MattermostProvider) GetUserName(s *sessions.SessionState) (string, error) {
	if s.AccessToken == "" {
		return "", errors.New("missing access token")
	}
	req, err := http.NewRequest("GET", p.ProfileURL.String()+"", nil)
...
func (p *MattermostProvider) ValidateSessionState(s *sessions.SessionState) bool {
